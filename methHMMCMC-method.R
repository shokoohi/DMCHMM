.marglike <- function(beta.est, Pm.est, Kv, Yv, nv, useweight){
    ...
    #nw <- log(nv+2)/log(sum(nv+2))
    #nw <- log(nv+2)
    #nw <- log(nv+2)/sum(log(nv+2))
    #nw <- rep(1,length(Yv))
    if(useweight){
        nw <- log(nv+2)
    }else{
...
.marglike.M <- function(X.estimate, Yv, nv, useweight){
    ...
    X.estimate[i],log=TRUE)), 0,dbinom(Yv[i],nv[i],X.estimate[i],log = TRUE))
    }

    return(mlike)
}

.HMMmcmc.M <- function(Yv, nv, Kv, old.prob, old.Uvec, old.P.U, nburn, nthin,
    nsamp, useweight){
    ...
    if(Itr == nburn){Prob.U.vec<-Prob.U.vec*0}

    #dtmp1 <- nv[1]*dbinom(Yv[1],nv[1],old.prob,log=T)
    #dtmp3 <- nv[1]*log(old.P.U[,old.Uvec[2]+1])
    ...
    u<-runif(1)
    old.prob[k+1]<-qbeta(FL+u*(FU-FL),ytot+1/2,ntot-ytot+1/2)
    }

    #print(noquote(c(sprintf("%5.0f",Itr),
    #sprintf("%5.5f", old.prob[2:(Kv+1)]))))
    ...
    x.samp[ico,] <- old.Uvec
    X.estimate[ico,] =
        c((Prob.U.vec/rowSums(Prob.U.vec))%*%t(old.prob))
    }
    }
...
.methHMMCMC <- function(object, useweight, nburn, nthin, nsamp,mc.cores)
    ...
    EstHMMSamMCMC.EM.W <-
    .runMCMC.M(c(assays(object)$methReads[,j]),c(assays(object)$totalReads[,j]),
    c(metadata(object)$K[j]), metadata(object)$Beta[[j]],
    c(assays(object)$methStates[,j]), metadata(object)$Pm[[j]], nburn, nthin,
    nsamp, useweight)
    ...
    methStates.new[,i] = as.integer(c(apply(totres2[[i]]$xMCMC$Prob.U.vec ,1,
    which.max)-1))
    }
...
setMethod("methHMMCMC",
    signature=c(object = "BSDMCs", useweight = "ANY", nburn = "ANY",
    nthin = "ANY", nsamp = "ANY", mc.cores = "ANY"),
    .methHMMCMC)
